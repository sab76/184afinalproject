...
class RNA_Dataset(Dataset):
    def __init__(self, df, mode='train', seed=2023, fold=0, nfolds=4, use_kfold=True, mask_only=False):
        ...
        if use_kfold:
            split = list(KFold(n_splits=nfolds, random_state=seed, shuffle=True).split(df_2A3))[fold][0 if mode=='train' else 1]
    ...
    def get_min_sequence_length(self):
        return min(len(seq) for seq in self.seq)
    
    def get_max_sequence_length(self):
        return max(len(seq) for seq in self.seq)    
